...
def render_admin_dashboard():
    ...
    """, unsafe_allow_html=True)
    try:
        # Fetch user name + email lookup map from Firestore users collection
        try:
            ...
            for u in user_docs:
                ...
                name  = d.get("name", "").strip()
                email = d.get("email", "").strip()
                # Show "Name (email)" if both exist, else whichever is available
                if name and email:
                    uid_to_name[u.id] = f"{name} ({email})"
                elif name:
                    uid_to_name[u.id] = name
                elif email:
                    uid_to_name[u.id] = email
        except Exception:
            uid_to_name = {}

        # Also build a fallback from Firebase Auth records already fetched above
        # (all_fb_users is available in this scope)
        for fb_u in all_fb_users:
            if fb_u.uid not in uid_to_name and fb_u.email:
        ...
        for doc in tx_docs:
            ...
            ts  = d.get("timestamp")
            uid = d.get("uid") or ""
            # Prefer stored field → Firestore users map → Firebase Auth → truncated uid
            performed_by = (
                d.get("performed_by")
                or uid_to_name.get(uid)
                or (uid[:12] + "…" if len(uid) > 12 else uid or "—")
            )
            tx_data.append({
